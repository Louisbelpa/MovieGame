      ...
      <div className="cdy-arena">
        ...
        <div className="cdy-ar-top">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span className="cdy-ar-help" title="Comment jouer">?</span>
          </div>
        </div>
...
const TUTO_INTRO = {
  film: 'Devine le film du jour à partir de son affiche.',
  serie: 'Devine la série du jour à partir d\'une image marquante.',
  face: 'Devine la personnalité du jour — sa photo reste floue jusqu\'au bout.',
};
function CandyTutorial({ game = 'film' }) {
  const cfg = GAME_CFG[game];
  const mediaNote = game === 'face'
    ? 'La photo reste floutée : appuie-toi sur l\'extrait biographique pour deviner.'
    : `Une ${game === 'serie' ? 'scène de la série' : 'affiche du film'} du jour s'affiche — à toi de la reconnaître.`;
  return (
    <div className="cdy-tuto">
      <div className="cdy-tuto-head">
        <span className="ic"><GlyphG game={cfg.glyph} size={30} /></span>
        <div><h3>Comment jouer</h3><p>{cfg.name} · {TUTO_INTRO[game]}</p></div>
        <span className="cdy-tuto-x">✕</span>
      </div>
      <div className="cdy-tuto-body">

        {/* 1 — essais */}
        <div className="cdy-tuto-demo">
          <div className="cdy-tuto-lbl"><span className="n">1</span><span className="tx">Tu as 5 essais</span></div>
          <div className="cdy-ar-slots">
            <span className="cdy-ar-slot used" /><span className="cdy-ar-slot used" />
            <span className="cdy-ar-slot" /><span className="cdy-ar-slot" /><span className="cdy-ar-slot" />
          </div>
          <div className="cdy-ar-input" style={{ marginTop: 11 }}>
            <span className="pill">{cfg.placeholder}</span>
          </div>
          <p className="cdy-tuto-note">L'autocomplétion t'aide à viser juste à chaque saisie.</p>
        </div>

        {/* 2 — proximité */}
        <div className="cdy-tuto-demo">
          <div className="cdy-tuto-lbl"><span className="n">2</span><span className="tx">Chaque erreur te rapproche</span></div>
          <div className="cdy-ar-board">
            <div className="cdy-ar-row wrong"><i>✕</i><span>{cfg.wrongs[0]}</span><em>{cfg.firstMeta}</em></div>
            <div className="cdy-ar-row correct"><i>✓</i><span>{cfg.answer}</span></div>
          </div>
          <p className="cdy-tuto-note">Un indice de proximité s'affiche après chaque tentative ratée.</p>
        </div>

        {/* 3 — indices */}
        <div className="cdy-tuto-demo">
          <div className="cdy-tuto-lbl"><span className="n">3</span><span className="tx">Des indices se débloquent</span></div>
          <div className="cdy-ar-hints">
            <div className="cdy-ar-hint on"><span className="hl">{cfg.hints[0].l}</span><span className="hv">{cfg.hints[0].v}</span></div>
            <div className="cdy-ar-hint on"><span className="hl">{cfg.hints[1].l}</span><span className="hv">{cfg.hints[1].v}</span></div>
            <div className="cdy-ar-hint"><span className="hl">Indice 3</span><span className="hv">🔒 verrouillé</span></div>
          </div>
          <p className="cdy-tuto-note">{mediaNote} Reviens chaque jour pour garder ta série 🔥.</p>
        </div>

      </div>
      <div className="cdy-tuto-foot">
        <button className="cdy-btn cdy-btn-lg cdy-btn-primary" style={{ width: '100%' }}>C'est parti !</button>
        <span className="cdy-tuto-dontshow">Ne plus afficher ce tutoriel</span>
      </div>
    </div>
  );
}
function CandyTutorialScreen({ game = 'film' }) {
  return (
    <div className={`cdy g-${game}`} style={{ position: 'relative', width: 760, minHeight: 900 }}>
      <div style={{ filter: 'blur(2px)', opacity: 0.5, pointerEvents: 'none' }}>
        <CandyGame game={game} state="start" />
      </div>
      <div className="cdy-tuto-scrim"><CandyTutorial game={game} /></div>
    </div>
  );
}

Object.assign(window, { CandyTutorial, CandyTutorialScreen });
