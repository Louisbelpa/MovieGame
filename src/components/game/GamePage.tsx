...
function TabBar({ mode }: { mode: 'film' | 'series' | 'wiki' }) {
  return (
    <nav className="cdym-tabs fixed bottom-0 left-0 right-0 z-30 lg:hidden">
      <a href="/films" className={`cdym-tab ${mode === 'film' ? 'on' : ''}`}>
        <span className="ic"><Film size={20} /></span>
        <span className="lb">Films</span>
      </a>
      {FEATURES.enableSeries && (
        <a href="/series" className={`cdym-tab ${mode === 'series' ? 'on' : ''}`}>
          <span className="ic"><Tv size={20} /></span>
          <span className="lb">Séries</span>
        </a>
      )}
      {FEATURES.enableWiki && (
        <a href="/wiki" className={`cdym-tab ${mode === 'wiki' ? 'on' : ''}`}>
          <span className="ic"><User size={20} /></span>
          <span className="lb">Perso</span>
        </a>
      )}
      <a href="/profile" className="cdym-tab">
        <span className="ic">
