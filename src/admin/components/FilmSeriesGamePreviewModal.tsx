...
import { fetchFilmGamePreview, fetchSeriesGamePreview } from '../api'
...
export function FilmSeriesGamePreviewModal({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    const fetch = mode === 'film' ? fetchFilmGamePreview(mediaId) : fetchSeriesGamePreview(mediaId)
    void fetch
      .then((data) => {
    ...
    >
      ...
      >
        <p className="text-xs text-amber-200/95 bg-amber-950/50 border border-amber-700/40 rounded-lg px-3 py-2">
          Aperçu basé sur la <strong className="font-semibold">fiche enregistrée</strong>. Enregistre puis rouvre l’aperçu après modification. Aucune tentative n’est enregistrée. Ordre des indices par défaut : année →{' '}
          {mode === 'film' ? 'réalisateur' : 'créateur'} → casting.{' '}
...
export function FilmSeriesPreviewOpenButton({
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className={`inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium border rounded-lg ${cls}`}
    >
